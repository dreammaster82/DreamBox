...
namespace core{
	class Util{
		...
		function mail($from, $from_text, $emailto, $subj, $body, $type="text/html", $file=''){
			...
			return mail($emailto, $subj, $body, $head);//, '-f '.$this->Core->globalConfig['email']['email_system']);
		}
		...
		function getrealpath($path){
			...
			return str_replace('//', '/', $add.implode('/', $real_arr));
		}
		...
		function memcacheSet($ind, $data, $class = 'all', $expire = 0){
			if($m = $this->Core->getClass('Memcache')){
				return $m->set($ind, $data, $class, $expire);
			}
		}
