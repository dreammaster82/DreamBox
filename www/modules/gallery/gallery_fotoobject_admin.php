...
namespace core\admin{
    ...
    class Gallery_FotoObject extends core\CContent{
        ...
        function moveItem($pId = 0){
            ...
            $out = array();
            if((int)$_REQUEST['what']){
                $out = $this->getItem(array('name', 'id'), (int)$_REQUEST['what']);
                $out['type'] = 0;
                ob_start();
        ...
        function moveItem2($pId = 0){
            $ret = $pId;
            if($_REQUEST['what']){
                $this->Db->queryExec('UPDATE '.$this->config['table'].' SET parent_id=? WHERE id=?', array((int)$_REQUEST['move_to'], $_REQUEST['what']));
            }
